...
func (storage *PostgresTransactionStorage) CreateTransaction(t *models.Transaction) (error) {
	...
	for _, posting := range t.Postings {
		...
		err := storage.db.QueryRow(postingQuery, transactionId, posting.AccountId, posting.Amount, posting.Comment, posting.IsComment).Scan(&postingId)
		if err != nil {
			log.Printf("Error %s when inserting row into postings table:\naccountid=%q", err, posting.AccountId)
			//return err
		}
	}
