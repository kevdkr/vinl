...
func (storage *PostgresTransactionStorage) CreateTransaction(t *models.Transaction) (error) {
	...
	for _, posting := range t.Postings {
		...
		postingQuery := "INSERT INTO postings (transactionid, accountid, amount, comment, is_comment) VALUES ($1, $2, $3, $4, $5) RETURNING id"
		...
		err := storage.db.QueryRow(postingQuery, transactionId, posting.Account.Id, posting.Amount, posting.Comment, posting.IsComment).Scan(&postingId)
		if err != nil {
			log.Printf("Error %s when inserting row into postings table:\naccountid=%q", err, posting.Account.Id)
			//return err
		}
	}
...
func (storage *PostgresTransactionStorage) GetTransactionById(id string) (*models.Transaction, error) {
	...
	var postings []models.Posting
	//postingsQuery := "SELECT id, transactionid, accountid, amount, comment, is_comment FROM postings WHERE transactionid = $1"
